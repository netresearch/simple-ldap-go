...
- **Docs are checked against the API, but only for call shape.** `scripts/check-docs-api.py` (CI job `docs-api`) verifies that every `l.`/`client.`/`ldapClient.` call in `docs/*.md` and `README.md` names a real `*LDAP` method and passes an argument count its signature accepts. It does not type-check arguments or compile the snippets, so a wrong *type* still slips through. Run it after renaming or re-signing a method.
